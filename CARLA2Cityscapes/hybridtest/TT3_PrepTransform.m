...
img_path = fullfile(dataDir, 'Cityscapes/ReOrg/Image-512/train');
label_path = fullfile(dataDir, 'Exp/C2C', runStr, 'SegColor-256');

path_coarse_library  = fullfile(dataDir, 'Exp/C2C', runStr, 'SegList-256');
path_response_proposal = fullfile(dataDir, 'Exp/C2C', runStr, 'SegMatch-256');
% path_response_proposal = '../testdata/searched_top10_segments/';


list = dir(fullfile(label_path,  '*.png'));
list = struct2cell(list);

list_train = dir(fullfile(img_path, '*.png'));
list_train = struct2cell(list_train);


save_path = fullfile(dataDir, 'Exp/C2C', runStr, 'PreTransform-512');
mkdir(save_path);
...
order_list = load('../../SIMS/matlab_code/order.mat');
...
for i = 1:nTest
  ...
  for c_ind = 1:19
     ...
     query = load_warper(fullfile(path_coarse_library, sprintf('%02d',c), [list{1,i}(1:end-4) '.mat']));
     ...
     data = load(fullfile(path_response_proposal, sprintf('%02d',c), [list{1,i}(1:end-4) '.mat']));
